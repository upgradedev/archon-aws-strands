    ...
    {intake ? <div className="intake-grid">
      <section className="panel intake"><h2>{replaceId ? `Correct ${replaceId}` : 'Read an email'}</h2><p>The bounded reader supports explicit EUR invoices and remittances. No live model call.</p>
        ...
        <form onSubmit={async e => { e.preventDefault(); if (await mutate('/intake', { body, replace_id: replaceId })) { setBody(''); setReplaceId(null); } }}>
          ...
          <div className="flex flex-wrap gap-3"><button className="primary" disabled={busy || !body.trim()}>{busy ? 'Reading…' : replaceId ? 'Read corrected source' : 'Read & post email'}</button>{replaceId ? <button type="button" className="secondary" onClick={() => { setReplaceId(null); setBody(''); }}>Cancel correction</button> : null}{source.status === 'refused' && source.kind === 'Receipt' ? <Resolution key={`${source.id}:${data.revision}`} source={source} data={data} busy={busy} mutate={mutate} /> : null}{source.resolution ? <p>Human resolution: {source.resolution.decision} · {source.resolution.note}. Original evidence retained.</p> : null}</div>
        </form>
      </section>
    </div> : null}

...
const navigation = [
  ['dashboard', 'Dashboard', 'See what is owed'], ['workspace', 'Workspace', 'Review & approve'], ['records', 'Records', 'Invoices & payments'], ['history', 'History', 'Decisions & receipts'],
] as const;
...
export function App() {
  ...
  useEffect(() => {
    if (busy || !data?.live?.job || !['queued', 'running'].includes(data.live.job.status)) return;
  ...
  return <div className="app-shell">
    ...
    <div className="workspace-shell"><header className="topbar"><div><span className="demo-pill">{data?.live ? 'LIVE AI · TEST DATA' : 'SYNTHETIC DEMO'}</span><span className="asof">As of {data?.as_of ?? 'Unknown'}{stale ? ' · Last known snapshot' : ''}</span></div><div className="flex gap-2"><button id="refresh-workspace" className="ghost" disabled={busy} onClick={() => void refresh()}>Refresh</button><button className="secondary small" disabled={busy} onClick={() => setNewSession(true)}>New workspace</button></div></header>
      <main id="main" tabIndex={-1} aria-busy={busy || providerPending}>
        ...
        {data ? <>{page === 'journey' ? <Journey key={session.current} data={data} busy={busy || providerPending} stale={stale} mutate={mutate} route={route} reviewEpoch={reviewEpoch} loadEvidence={() => request<Bundle>('/evidence', session.current)} /> : page === 'dashboard' ? <Dashboard data={data} stale={stale} /> : page === 'workspace' ? <Queue key={session.current} data={data} busy={busy || providerPending} mutate={mutate} route={route} stale={stale} reviewEpoch={reviewEpoch} loadEvidence={() => request<Bundle>('/evidence', session.current)} /> : page === 'records' ? <Documents key={session.current} data={data} busy={busy || providerPending || stale} mutate={mutate} route={route} /> : page === 'history' ? <Activity key={session.current} data={data} loadEvidence={() => request<Bundle>('/evidence', session.current)} /> : <div className="empty"><h1 tabIndex={-1}>Page not found</h1><p><a href="#/dashboard">Return to Dashboard</a></p></div>}</> : !error ? <div className="loading" role="status"><div className="loading-bar" /><h1>Opening your ledger</h1><p>Creating or reading your isolated demo session.</p></div> : null}
        {data?.demo_seed ? <section className="notice" aria-label="Populated fictional demo"><strong>Your demo is ready to explore.</strong><p>Five fictional source emails, parsed with local rules. Metrics come from the ledger, not AI-generated totals. No AI report or email was generated by loading.</p><a href="#/journey">Next: review the remaining balance and run the agent →</a></section> : data && data.sources.length === 0 ? <section className="notice" aria-label="Empty workspace help"><strong>Start with a working example.</strong><p>This workspace is empty. Explore populated books, or add your first invoice below.</p><a className="secondary" href="#/demo">Explore populated demo →</a></section> : null}

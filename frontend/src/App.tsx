...
export function App() {
  ...
  return <div className="app-shell">
    ...
    <aside className="sidebar"><a href="#/dashboard" className="brand"><span className="brand-mark" aria-hidden="true">A</span><span>ARCHON<small>THE INBOX LEDGER</small></span></a>
    ...
    <div className="workspace-shell"><header className="topbar"><div><span className="demo-pill">SYNTHETIC DEMO</span><span className="asof">As of {data?.as_of ?? 'Unknown'}{stale ? ' · Last known snapshot' : ''}</span></div><div className="flex gap-2"><button className="ghost" disabled={busy} onClick={() => void refresh()}>Refresh</button><button className="secondary small" disabled={busy} onClick={() => setNewSession(true)}>New workspace</button></div></header>
      <main id="main" tabIndex={-1} aria-busy={busy}>
        ...
        {data ? <>{page === 'dashboard' ? <Dashboard data={data} stale={stale} /> : page === 'workspace' ? <Queue data={data} busy={busy} mutate={mutate} route={route} stale={stale} reviewEpoch={reviewEpoch} /> : page === 'records' ? <Documents data={data} busy={busy || stale} mutate={mutate} route={route} /> : page === 'history' ? <Activity data={data} /> : <div className="empty"><h1 tabIndex={-1}>Page not found</h1><p><a href="#/dashboard">Return to Dashboard</a></p></div>}</> : !error ? <div className="loading" role="status"><div className="loading-bar" /><h1>Opening your ledger</h1><p>Creating or reading your isolated demo session.</p></div> : null}

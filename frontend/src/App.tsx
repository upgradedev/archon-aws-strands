...
export function App() {
  ...
  useEffect(() => {
    if (page !== 'welcome' && !opened.current) { opened.current = true; void refresh(); }
  }, [page]);
  ...
  return <div className="app-shell">
    ...
    <div className="workspace-shell"><header className="topbar"><div><span className="demo-pill">SYNTHETIC DEMO</span><span className="asof">As of {data?.as_of ?? 'Unknown'}{stale ? ' · Last known snapshot' : ''}</span></div><div className="flex gap-2"><button id="refresh-workspace" className="ghost" disabled={busy} onClick={() => void refresh()}>Refresh</button><button className="secondary small" disabled={busy} onClick={() => setNewSession(true)}>New workspace</button></div></header>
      <main id="main" tabIndex={-1} aria-busy={busy}>
        ...
        {data ? <>{page === 'journey' ? <Journey key={session.current} data={data} busy={busy} stale={stale} mutate={mutate} route={route} reviewEpoch={reviewEpoch} loadEvidence={() => request<Bundle>('/evidence', session.current)} /> : page === 'dashboard' ? <Dashboard data={data} stale={stale} /> : page === 'workspace' ? <Queue key={session.current} data={data} busy={busy} mutate={mutate} route={route} stale={stale} reviewEpoch={reviewEpoch} loadEvidence={() => request<Bundle>('/evidence', session.current)} /> : page === 'records' ? <Documents key={session.current} data={data} busy={busy || stale} mutate={mutate} route={route} /> : page === 'history' ? <Activity key={session.current} data={data} loadEvidence={() => request<Bundle>('/evidence', session.current)} /> : <div className="empty"><h1 tabIndex={-1}>Page not found</h1><p><a href="#/dashboard">Return to Dashboard</a></p></div>}</> : !error ? <div className="loading" role="status"><div className="loading-bar" /><h1>Opening your ledger</h1><p>Creating or reading your isolated demo session.</p></div> : null}

...
export function Approvals({ data, busy, mutate, mode = 'all', selectedInvoice, stale = false }: { data: Workspace; busy: boolean; mutate: Mutate; mode?: 'draft' | 'terms' | 'all'; selectedInvoice?: string; stale?: boolean }) {
  ...
  return <>
    ...
    {mode !== 'terms' ? <section className="panel" id="collection-draft" tabIndex={-1}><div className="panel-heading"><div><h2>Collection draft</h2><p>Every money claim is re-derived at approval. Drafts expire after 30 minutes.</p></div><Badge tone="amber">Simulated delivery</Badge></div>
      {draft ? <div className="draft-grid"><article className="email"><dl><dt>To</dt><dd>{draft.recipient}</dd><dt>Subject</dt><dd>{draft.subject}</dd><dt>Invoice</dt><dd><a href={`#/documents?source=${encodeURIComponent(draft.invoice_id)}`}>{draft.invoice_id} · Source evidence ↗</a></dd></dl><pre>{draft.body}</pre>{mode === 'draft' ? <details className="evidence-disclosure"><summary>Linked ledger evidence</summary><DraftEvidence data={data} invoiceId={draft.invoice_id} /></details> : <DraftEvidence data={data} invoiceId={draft.invoice_id} />}<details><summary>Exact content fingerprint</summary><code className="fingerprint">{draft.fingerprint}</code></details></article>

...
export function reconciliation(data: Workspace, invoice: string, stale: boolean, now: number) {
  ...
  const credited = cents(balance?.credited ?? '0.00');
  const hasCredit = credited !== null && credited > 0n;
  ...
  const before = latest && amount !== null && amount > 0n && gross !== null && settled !== null && outstanding !== null
    && credited === 0n && cents(total) === settled && gross - settled === outstanding ? decimal(outstanding + amount) : null;
  ...
  if (outstanding === 0n && hasCredit) return { ...common, title: 'Closed with credit. No chase.', why: 'Posted credits and any recorded cash receipts cover this invoice. A credit is a balance adjustment, not a payment received.', action: 'Inspect invoice and credit sources', href: `#/records?q=${encodeURIComponent(invoice)}` };
  ...
  return { ...common, title: hasCredit ? 'Credit evidence changed the decision' : latest ? 'Payment evidence changed the decision' : 'Prepare a decision from these books', why: status === 'expired' || status === 'unavailable'

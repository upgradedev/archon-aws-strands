...
export function reconciliation(data: Workspace, invoice: string, stale: boolean, now: number) {
  ...
  if (blocked) return { ...common, title: 'Collection is paused for this case', why: `The queue records: ${blocked.reason}. A payment promise changes timing, not the ledger balance.`, action: 'Review case terms', href: workspaceLink(invoice, null, 'terms') };

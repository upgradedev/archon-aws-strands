...
import { cents, draftState, linkedSources, reasonTarget, routeInfo, workspaceLink } from './ledger';
...
function SourceStep({ data, payment, invoice, disabled, mutate, finish, hasPayment }: {
  ...
  return <form className="journey-editor" onSubmit={async event => {
    ...
    <div className="journey-editor-body"><label htmlFor="journey-source">Email headers and plain-text body</label><textarea id="journey-source" rows={11} value={body} disabled={disabled} maxLength={32000} onChange={event => { setBody(event.target.value); setValidation(''); }} aria-describedby="journey-format" />
      <p id="journey-format" className="field-help">Supported: plain-text English email with explicit sender, recipient, invoice reference, ISO dates or full English month names, and EUR amounts such as 1,234.56 or 1.234,56. Net, VAT and total must agree. Payments also need the original transfer ID. {data.live ? 'Real semantic extraction with conservative source checks: use explicit ISO dates and amounts such as 1,234.56. Model suggestions can be refused.' : 'This is a bounded rule reader, not general document AI.'} No PDFs, scans or attachments.</p>
    ...
    <div className="guided-desk"><div className="guided-task">
      ...
      {!blocked && step === 2 ? <>
        <section className="journey-review"><p className="eyebrow">STEP 3 / REVIEW THE NEXT ACTION</p><h2 id="guided-review-heading" tabIndex={-1}>{needsDraft ? decision.title : 'Read the exact reminder before deciding'}</h2><p>{needsDraft ? decision.why : 'A recorded payment is deducted once. The graph reads the books; it does not verify your bank or decide for you.'}</p>
          ...
          {needsDraft ? <><button className="primary" disabled={busy || !target.id || target.id !== invoice || !!target.issue} onClick={() => void mutate('/reason')}>Run Strands & prepare draft</button><p className="field-help">{target.id !== invoice ? 'This invoice is not currently the collection priority. Open the collection desk to inspect holds or another case.' : target.issue ? 'Correct the invoice evidence in Records before preparing a draft.' : (data.live ? 'Runs real Bedrock and Strands. No email is sent by drafting.' : 'Runs the real Strands graph using a scripted model. No email is sent.')}</p></> : null}
      ...
      {!blocked && step === 3 && receipt ? <section className="journey-result" aria-label="Guided check outcome"><Badge tone={receipt.state === 'provider-accepted' ? 'green' : 'amber'}>{data.live?.mail ? 'SES' : 'Simulated'} · {receipt.state}</Badge><h2 id="guided-outcome-heading" tabIndex={-1}>{receipt.state === 'provider-accepted' ? 'Your decision is recorded.' : 'Read this outcome before acting again.'}</h2><p>{data.receipt_states[receipt.state] ?? 'This state has no known explanation. Inspect the receipt before taking any further action.'}</p><p>{data.live?.mail ? 'Inspect the SES receipt for this real attempt. Approval did not collect money or reduce the debt.' : 'No real email was sent. Approval did not collect money or reduce the debt.'} The current outstanding balance remains <strong>{money(balance?.outstanding)}</strong>.</p><a className="primary" href="#/dashboard">Return to overview →</a><div className="journey-actions"><a href="#/history">Inspect the saved receipt →</a><a href={href('payment')}>Add new payment evidence →</a></div><EvidenceBundle revision={data.revision} load={loadEvidence} /></section> : null}
      ...
      {balance ? <><h2>{balance.counterparty}</h2><p>{invoice} · Due {balance.due}</p><dl><div><dt>Invoiced</dt><dd>{money(balance.gross)}</dd></div><div><dt>Recorded receipts</dt><dd>{money(balance.settled)}</dd></div>{(cents(balance.credited) ?? 0n) > 0n ? <div><dt>Credited · not cash</dt><dd>{money(balance.credited)}</dd></div> : null}<div className="journey-total"><dt>Still owed</dt><dd>{money(balance.outstanding)}</dd></div></dl><p>Retained source evidence, not verified bank settlement.</p><details><summary>Inspect original sources ({sources.length})</summary>{sources.map(source => <div className="journey-source" key={source.id}><a href={`#/records?source=${encodeURIComponent(source.id)}`}>{source.id} · {source.kind}</a><pre>{source.body}</pre></div>)}</details></> : <><h2>No invoice posted yet</h2><p>Your real session balance appears here after you submit an invoice. We do not insert illustrative totals into your books.</p></>}
      <div className="journey-boundary"><strong>You stay in control</strong><p>{data.live ? 'Fictional examples · real Bedrock model · controlled SES outbox.' : 'Synthetic examples · scripted model · simulated outbox.'} A new payment invalidates an old draft and its review confirmation.</p></div><a href="#/dashboard">Back to overview →</a>
    </aside></div>
  </>;
}

...
export function Journey({ data, busy, stale, mutate, route, reviewEpoch, loadEvidence }: {
  ...
  return <>
    ...
    <div className="guided-desk"><div className="guided-task">
      ...
      {!blocked && step === 3 && receipt ? <section className="journey-result" aria-label="Guided check outcome"><Badge tone={receipt.state === 'provider-accepted' ? 'green' : 'amber'}>Simulated · {receipt.state}</Badge><h2>{receipt.state === 'provider-accepted' ? 'Your decision is recorded.' : 'Read this outcome before acting again.'}</h2><p>{data.receipt_states[receipt.state] ?? 'This state has no known explanation. Inspect the receipt before taking any further action.'}</p><p>No real email was sent. Approval did not collect money or reduce the debt. The current outstanding balance remains <strong>{money(balance?.outstanding)}</strong>.</p><a className="primary" href="#/dashboard">Return to overview →</a><div className="journey-actions"><a href="#/history">Inspect the saved receipt →</a><a href={href('payment')}>Add new payment evidence →</a></div><EvidenceBundle revision={data.revision} load={loadEvidence} /></section> : null}
